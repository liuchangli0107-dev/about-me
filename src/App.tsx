...
const App: React.FC = () => {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100 selection:text-blue-700">
      ...
      <nav className="fixed w-full bg-white/80 backdrop-blur-md z-50 border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex space-x-8 font-medium">
            ...
            <a href="#skills" className="hover:text-blue-600 transition-colors">框架與技術</a>
            <a href="#experience" className="hover:text-blue-600 transition-colors">實戰經驗</a>
            <a href="#philosophy" className="hover:text-blue-600 transition-colors">背景與理念</a>
          </div>
      ...
      <section id="philosophy" className="py-20 bg-slate-900 text-white overflow-hidden relative">
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            {/* Left side: Education */}
            <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl h-full">
              <h3 className="text-2xl font-bold mb-6 flex items-center">
                <GraduationCap className="mr-3 text-blue-400" size={28} />
                <span>【教育背景】</span>
              </h3>
              <div>
                <h4 className="font-bold text-slate-100 text-lg">{education.degree}</h4>
                ...
                {philosophies.map((philosophy, idx) => {
                  ...
                  return (
                    <div className="flex gap-4" key={idx}>
                      ...
                      <div>
                        <h4 className="font-bold mb-1 text-slate-100">{philosophy.title}</h4>
      ...
      <footer className="py-20 border-t border-slate-200">
        <div className="max-w-6xl mx-auto px-6 flex flex-col items-center">
          <div className="flex flex-wrap justify-center gap-6 mb-12">
            <a href="mailto:liuchangli0107@gmail.com" className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-full hover:border-blue-500 transition-colors">
              <Mail className="text-blue-600" size={20} />
              <span className="font-medium text-slate-700">liuchangli0107@gmail.com</span>
            </a>
            <div className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-full">
              <Phone className="text-blue-600" size={20} />
              <span className="font-medium text-slate-700 font-mono">0928-323-276</span>
            </div>
          </div>
          <div className="flex gap-4 mb-8">
            <a href="https://github.com/charlieliu" target="_blank" rel="noreferrer" className="p-3 bg-slate-100 rounded-full hover:bg-slate-200 transition-all">
              <Github size={24} />
            </a>
          </div>
          <p className="text-slate-400 text-sm">© 2026 Chang-Li Liu. Built with Structured Logic & TypeScript.</p>
        </div>
      </footer>
    </div>
  );
